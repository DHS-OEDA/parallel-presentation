...
1.  Cores, threads, and vCPUs
2.  Serialization
3.  Embarassingly parallel
...
I have a *list of Case IDs.* I want to *query the Data Warehouse for attributes about the case*, which are then used to query the Data Warehouse for all case notes. After collecting all of these notes and processing the statements within, I want to score the statements with 1 or more predictive models, and write the results to a local `duckdb` database.
...
`tryCatch()` and `withCallingHandlers` and the `logging` package to
...
`withCallingHandlers` gives more detail on the error by using a condition system
(stored as S3-style objects in R).
